...
# Decode Google Cloud credentials from base64 environment variable
gcp_credentials_b64 = os.environ.get("GOOGLE_CLOUD_CREDENTIALS_B64")
...
storage_client = storage.Client(credentials=service_account.Credentials.from_service_account_info(gcp_credentials))
...
def upload_audio():
    ...
    transcript = transcribe_audio(f"gs://{BUCKET_NAME}/{cloud_filename}")
...
def transcribe_audio(gcs_uri):
    ...
    client = speech.SpeechClient(credentials=service_account.Credentials.from_service_account_info(gcp_credentials))
